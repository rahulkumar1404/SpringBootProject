...
import com.routemaster.repository.ReservationRepository;
...
public class ReservationServiceImpl implements ReservationService{
	...
	public Reservation addReservation(ReservationDTO dto, String key) throws ReservationException {
		...
		if(userSession == null)
			throw new ReservationException("Invalid Session key for user");
		...
		if(optional.isEmpty())
			throw new ReservationException("User not found with the session key");
	...
	public Reservation viewReservation(Integer rid, String key) throws ReservationException {
		...
		if(currentAdminSession == null)
			throw new ReservationException("Invalid admin login key");
	...
	public List<Reservation> getAllReservation(String key) throws ReservationException {
		...
		if(session == null)
			throw new ReservationException("Please provide valid admin login key");
	...
	public List<Reservation> viewReservationByUserId(Integer uid, String key) throws ReservationException {
		...
		if(currentAdminSession == null &&currentUserSession == null)
			throw new ReservationException("Invalid login key");
		
		Optional<User> optional = userRepository.findById(uid);
		
		if(optional.isEmpty())
			throw new ReservationException("User not find with given user id: "+uid);
		...
		if(reservations.isEmpty())
			throw new ReservationException("Reservation not found for this user");
	...
	public Reservation deleteReservation(Integer rid, String key) throws ReservationException {
		...
		if(currentUserSession == null)
			throw new ReservationException("Invalid session key");
		...
		if (reservation.getJourneyDate().isBefore(LocalDate.now())) {
			throw new ReservationException("Reservation Already Expired");
		}
	...
	public Reservation updateReservation(Integer rid, ReservationDTO dto, String key) throws ReservationException {
		...
		if(availableSeats < dto.getBookedSeat())
			throw new ReservationException("Only " + availableSeats);
